{{-- File narasi BARU untuk data dasarian GABUNGAN (Prediksi + Peluang) --}}
@php
    $namaLokasi = "<strong>" . e($locationName) . "</strong>";
    // $listDetailsPred, $listDetailsProb, dan $defaultProbThreshold dikirim dari Controller
@endphp
...
@switch($templateId)
    ...
    @case(1)
        ...
        {{-- Bagian Peluang (Hanya tampil jika ada) --}}
        @if($listDetailsProb)
    ...
    @case(3)
        {{-- Versi 3 --}}
        Grafik bagian kiri ini menggambarkan prakiraan curah hujan untuk setiap dasarian di wilayah {!! $namaLokasi !!}. Diprediksi curah hujan {!! $listDetailsPred !!}

        {{-- Bagian Peluang (Hanya tampil jika ada) --}}
        @if($listDetailsProb)
            <br><br>Sedangkan grafik bagian kanan menampilkan hasil prakiraan peluang curah hujan <span class="prob-threshold">{!! $defaultProbThreshold !!}</span>. Selama tiga dasarian mendatang, peluang curah hujan tersebut masing-masing sebesar <span class="prob-details">{!! $listDetailsProb !!}</span>.
        @endif
        @break
    ...
    @default
        ...
        {{-- Bagian Peluang (Hanya tampil jika ada) --}}
        @if($listDetailsProb)
            <br><br>Grafik di sisi kanan memperlihatkan prakiraan probabilitas curah hujan <span class="prob-threshold">{!! $defaultProbThreshold !!}</span>. Dalam tiga dasarian mendatang, peluang terjadinya curah hujan tersebut masing-masing adalah <span class="prob-details">{!! $listDetailsProb !!}</span>.
        @endif
@endswitch